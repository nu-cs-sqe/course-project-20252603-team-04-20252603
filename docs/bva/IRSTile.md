...
- **TC1: getName returns IRS tile type** ( :x: )
  - **State of the system**: IRS tile exists
  - **Expected output**: returns `TileType.IRS`

- **TC2: getName does not return null** ( :x: )
  - **State of the system**: IRS tile exists
  - **Expected output**: returned value is not `null`

- **TC3: getName repeated calls are consistent** ( :x: )
...
- **TC4: Land on IRS with balance greater than tax amount** ( :x: )
...
- **TC11: Land on IRS does not transfer money to another player** ( :x: )
